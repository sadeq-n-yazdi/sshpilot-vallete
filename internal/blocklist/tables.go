...
// finishes, every stroke-shaped source has already converged on i: the ASCII
// digit via leetspeak; ASCII capital I via the case fold; fullwidth "１",
// circled "①", superscript "¹" and the mathematical digits via foldRange and
// confusables chaining into leetspeak; and dotless ı, Greek iota and script
// capital ℐ via confusables. Keying the ambiguity on the fold's OUTPUT covers
// all of them with one entry, and -- more usefully -- covers any future table
// entry that folds a new stroke-shaped codepoint to i without this table
// needing to learn about it. Keying on each source would need an entry per
// source and would silently miss the next one added.
//
