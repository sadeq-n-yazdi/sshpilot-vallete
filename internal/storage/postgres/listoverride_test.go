...
// Postgres is free to return rows in plan order, so the ORDER BY is doing real
// work here rather than pinning an incidental one.
...
// needs to be interpretable is required. A row missing one of these could not
// be replayed as policy, and the fail-open direction makes a silent accept
// unacceptable.
...
// The CHECK constraints stop the application writing such a row, so this drops
// the offending constraint to install the row the application layer is designed
// to make unwritable -- the case where the table was modified out from under the
// service. Returning the row would let replay act on a value it cannot
// interpret, and for a tombstone that is the fail-open direction: a removal read
// as anything else lets the seed resurrect the entry.
func TestListOverrideReadRefusesAnUninterpretableRow(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"unknown list": {
			constraint: "list_overrides_list_check",
			list:       "denylist", state: string(domain.ListOverridePresent),
		},
		"unknown state": {
			constraint: "list_overrides_state_check",
			list:       string(domain.ListKindAllowlist), state: "pending",
		},
	}
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			...
			// The CHECK constraint refuses a value outside the enum, so the row
			// the decode path must reject cannot be inserted while the
			// constraint stands. Dropping it models the same situation the
			// adapter guards against: a value in the column that the domain does
			// not recognize.
			if _, err := s.db.ExecContext(ctx,
				`ALTER TABLE list_overrides DROP CONSTRAINT `+tc.constraint); err != nil {
				t.Fatalf("drop %s: %v", tc.constraint, err)
			}
			_, err := s.db.ExecContext(ctx,
				`INSERT INTO list_overrides (`+listOverrideColumns+`) VALUES ($1, $2, $3, $4, $5, $6)`,
...
func TestListOverrideRepoIsWiredIntoRepos(t *testing.T) {
	...
	if got := s.Repos().ListOverrides; got == want {
		t.Fatal("Repos().ListOverrides is nil, want the PostgreSQL adapter")
	}
}
