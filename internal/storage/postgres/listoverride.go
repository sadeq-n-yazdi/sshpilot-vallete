...
// listOverrideRepo is the PostgreSQL adapter for durable runtime edits to the
...
// repository.ListOverrideRepository and migration0011ListOverrides. It mirrors
// the SQLite adapter statement for statement so the two engines compose the
// same policy from the same rows.
//
...
// refuse and which exemptions stand. A row that cannot be interpreted must
// never be replayed as though it could be, and the direction of a
// misinterpretation is exactly the fail-open one this table exists to close --
// a tombstone read as anything other than a tombstone lets the seed resurrect a
// removed entry. The CHECK constraints already refuse such a row on write, so a
// decode failure here means the table was modified out from under the
// application, which is precisely when returning the row would be worst.
// Failing the read makes the caller fail closed.
...
// the database happened to return rows, and Postgres -- unlike SQLite -- is
// free to return rows in whatever order the plan produces, so the ORDER BY is
// doing real work here rather than pinning an incidental one. It returns a nil
// slice when there are no overrides.
//
