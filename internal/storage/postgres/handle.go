...
func (r *handleRepo) refuseIfFoldStale(ctx context.Context) error {
	const probe = `SELECT 1 FROM handles WHERE fold_version <> $1 LIMIT 1`
	var one int
...
func (r *handleRepo) ListStaleFolds(ctx context.Context, currentVersion int) ([]domain.Handle, error) {
	q := `SELECT ` + handleColumns + ` FROM handles WHERE fold_version <> $1
ORDER BY created_at ASC, id ASC`
...
func (r *handleRepo) SetFold(ctx context.Context, id domain.HandleID, fold string, version int, updatedAt time.Time) error {
	const q = `UPDATE handles SET name_fold = $1, fold_version = $2, updated_at = $3
WHERE id = $4`
...
SET state = $1, quarantine_until = NULL, flagged_for_review = $2, updated_at = $3
WHERE id = $4`
	res, err := r.e.ExecContext(ctx, q,
		string(domain.NameStateQuarantined), true, encTime(updatedAt), string(id))
	if err != nil {
		return mapError(err)
	}
	return requireAffected(res)
