...
// accessKeyRepo is the SQLite AccessKeyRepository.
//
...
// party that ever holds the plaintext. A duplicate primary key maps to
// domain.ErrConflict.
...
FROM access_keys WHERE id = ? AND owner_id = ?`
...
// for a set by guessing or replaying that set's id.
func (r *accessKeyRepo) ListByKeySet(ctx context.Context, ownerID domain.OwnerID, setID domain.KeySetID) ([]domain.AccessKey, error) {
	const q = `SELECT ` + accessKeyColumns + `
FROM access_keys WHERE owner_id = ? AND key_set_id = ? ORDER BY id ASC`
...
func (r *accessKeyRepo) ListByOwner(ctx context.Context, ownerID domain.OwnerID) ([]domain.AccessKey, error) {
	const q = `SELECT ` + accessKeyColumns + `
FROM access_keys WHERE owner_id = ? ORDER BY id ASC`
...
func (r *accessKeyRepo) MarkRotated(ctx context.Context, ownerID domain.OwnerID, id domain.AccessKeyID, replacedBy domain.AccessKeyID, graceUntil time.Time) error {
	const q = `UPDATE access_keys SET status = ?, replaced_by_id = ?, grace_until = ?
...
// sweep makes progress rather than re-reading one ambiguous page.
func (r *accessKeyRepo) ListExpiredGrace(ctx context.Context, now time.Time, limit int) ([]domain.AccessKey, error) {
	...
	const q = `SELECT ` + accessKeyColumns + `
FROM access_keys WHERE status = ? AND grace_until IS NOT NULL AND grace_until <= ?
ORDER BY grace_until ASC, id ASC LIMIT ?`
