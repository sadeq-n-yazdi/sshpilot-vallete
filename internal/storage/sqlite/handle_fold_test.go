...
func foldOf(t *testing.T, s *Store, id string) (string, int) {
	...
	err := s.db.QueryRowContext(context.Background(),
		`SELECT name_fold, fold_version FROM handles WHERE id = ?`, id).Scan(&fold, &ver)
	if err != nil {
...
func TestHandleRegisterFailsClosedOnStaleFold(t *testing.T) {
	...
	// A first claim registers cleanly against an empty table.
	...
	// The refused claim left no row behind.
...
func TestHandleSetFoldConflict(t *testing.T) {
	...
	// h-1's fold is "alice"; forcing h-2 onto it collides.
