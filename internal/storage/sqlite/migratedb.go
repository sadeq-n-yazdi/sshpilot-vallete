...
// migration runner can execute against SQLite without importing database/sql or
// this driver. It is a thin pass-through: it does not translate errors into
// domain sentinels, because the migration runner has its own error handling and
// treats every database failure the same way.
...
// Begin starts a transaction. Because the handle was opened with
// _txlock=immediate, the driver begins it as BEGIN IMMEDIATE and acquires the
// write lock up front.
...
// Rollback after a successful Commit is a no-op that returns nil, so callers may
// defer Rollback unconditionally.
